/** Bounded, no-follow OpenCode Desktop version extraction from a physical Electron ASAR. */
...
type RangeReader = typeof readProviderRegularFileRangeNoFollow;
...
export function readOpencodeAppVersion(
    archivePath: string,
    readRange: RangeReader = readProviderRegularFileRangeNoFollow,
): string | null {
    try {
        const prefix = readExactRange(readRange, archivePath, 0, PREFIX_BYTES);
        ...
        const header = readExactRange(readRange, archivePath, 8, headerSize);
        ...
        const manifest = readExactRange(readRange, archivePath, packageStart, packageEntry.size);
        ...
        const repeatedPrefix = readExactRange(readRange, archivePath, 0, PREFIX_BYTES);
        const repeatedHeader = readExactRange(readRange, archivePath, 8, headerSize);
        const repeatedManifest = readExactRange(readRange, archivePath, packageStart, packageEntry.size);
        if (
            repeatedPrefix === null ||
            repeatedHeader === null ||
        ...
        return packageManifest?.name === "@opencode-ai/desktop" && isNumericDottedVersion(versionText) ? versionText : null;
    } catch {
        return null;
    }
}
...
function readExactRange(
    ...
    offset: number,
    length: number,
): ProviderRegularFileRangeRead | null {
    const result = readRange(path, offset, length);
