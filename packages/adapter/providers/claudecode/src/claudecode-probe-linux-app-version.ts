/** Bounded, no-follow Claude App version extraction from the physical Electron ASAR. */
 ...
 */
export function readClaudeLinuxAppVersion(archivePath: string): string | null {
    try {
        ...
        return packageManifest?.name === "@ant/desktop" &&
            packageManifest.productName === "Claude" &&
            isNumericDottedVersion(versionText)
            ? versionText
            : null;
    } catch (error) {
        if (inspectFilesystemFailure(error).failureKind === "permission_denied") throw error;
        return null;
    }
}
