/** Reusable exact native/canonical Skill graph fixtures for Cursor target tests. */
...
import { createCursorAppSkillTargetSupports, createCursorCliSkillTargetSupports } from "../src/cursor-target-skill";

export type SkillVariant = "project_folder" | "global_config_folder" | "global_directory_folder";

export const HASH = `sha256:${"8".repeat(64)}` as Sha256Digest;
export const BUILD_HASH = "sha256:eed61c5224668c9236334c4c68936a16aecc37374b592f59e31eb50433817831";
export const APP_LINUX_BUILD = "sha256:98c0fc2885636738e986e01af8f9c5229dad4b2d7510bc489c9ffc0924da4904";
export const APP_WIN32_BUILD = "sha256:4defe15e408c98082ee766f761ec77f9504f74727f57446a135b87bb44a4254e";
...
export const ENTRY_BODY = "Use the bundled resources. OAAM_CURSOR_SKILL_GRAPH_41A7D2\n";
export const CHANGED_ENTRY_BODY = ENTRY_BODY.replace("41A7D2", "99C4E1");
export const SCRIPT = "print('OAAM_CURSOR_RESOURCE_V1')\n";
export const CHANGED_SCRIPT = "print('OAAM_CURSOR_RESOURCE_V2')\n";
export const REFERENCE = "OAAM_CURSOR_REFERENCE_V1\n";
export const CHANGED_REFERENCE = "OAAM_CURSOR_REFERENCE_V2\n";
export const BINARY = Uint8Array.of(0, 255, 1, 2);
export const CHANGED_BINARY = Uint8Array.of(0, 255, 4, 5);

export const projectSchema = cursorProvider.targetContextSchemas.find(
    (row) => row.targetContextSchemaId === "CURSOR_AGENT_CLI_PROJECT_TARGET_V1",
);
if (projectSchema === undefined) throw new Error("Cursor project target schema is missing");
export const supports = createCursorCliSkillTargetSupports({
...
export const appSupports = createCursorAppSkillTargetSupports({
    adapterVersion: cursorProvider.version,
    agentRuntimes: cursorProvider.agentRuntimes,
...
export const VARIANTS: readonly SkillVariant[] = ["project_folder", "global_config_folder", "global_directory_folder"];
...
export function canonicalMaterializationFixture(
    variant: SkillVariant,
    losses: RenderDegradationKind[] = [],
): RenderAnalysisInput {
    const support = supportFor(variant);
...
export function skillBoundary(variant: SkillVariant): string {
    if (variant === "global_config_folder") return "skills/oaam-phase58-global-config-skill";
    if (variant === "global_directory_folder") return "oaam-phase58-global-direct-skill";
    return ".agents/skills/oaam-phase58-project-skill";
}

export function skillName(variant: SkillVariant): string {
    if (variant === "global_config_folder") return "oaam-phase58-global-config-skill";
    if (variant === "global_directory_folder") return "oaam-phase58-global-direct-skill";
    return "oaam-phase58-project-skill";
}
...
export function canonicalEntryPattern(variant: SkillVariant): RegExp {
    ...
    return /^\.cursor\/skills\/oaam-skill-11111111\/SKILL\.md$/;
}

export function skillHeader(variant: SkillVariant): string {
    return `${[
        "---",
        "# preserve Cursor Skill layout",
        `name: ${skillName(variant)}`,
        "description: OAAM Cursor graph Skill",
...
export function skillCanonical(name: string): Extract<AssetKindTypeDataV2, { kind: "Skill" }> {
    return {
        ...
        typeData: {
            ...
            name,
            description: "OAAM Cursor graph Skill",
            whenToUse: "OAAM Cursor graph Skill",
...
export function semantic(kind: string, versionId: string, subjectKind: "asset" | "file", fileId?: string) {
    return {
        ...
        consumerAgentRuntimeId: "CURSOR_AGENT_CLI" as const,
        subject:
            subjectKind === "asset"
